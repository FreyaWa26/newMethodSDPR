...
void init_state(Dat *dat, MCMC_state *state) {
	...
	state->pi[i] = 1.0 / state->n_cluster;
    ...
    state->p = (double *)malloc(sizeof(double)*2);
    state->tau = 1.0;
...
void MCMC_state::sample_tau(const Dat *dat){
	...
	for (size_t i = 0; i < dat->n_ind; i++) {
        sum_of_squares += std::pow(residual[i], 2);
    }
	
    double rate = sum_of_squares / 2.0 + b0k;
	//cout << shape<<"rate" <<rate<< endl;
	gsl_rng *r = gsl_rng_alloc(gsl_rng_mt19937);
	tau = gsl_ran_gamma(r, shape, 1.0/rate);
	//cout << "tau done" << endl;
}
...
void MCMC_state::sample_V() {
    vector<double> a(n_cluster-1);
    a[n_cluster-2] = suffstats[n_cluster-1];
    for (int i=n_cluster-3; i>=0; i--) {
	a[i] = suffstats[i+1] + a[i+1];
    }
	gsl_rng *r = gsl_rng_alloc(gsl_rng_mt19937);
    for (size_t i=0; i<n_cluster-1; i++) {
...
void MCMC_state::update_p() {
    vector<double> cumprod(n_cluster-1);
    
    cumprod[0] = 1 - V[0];

    for (size_t i=1; i<n_cluster-1; i++) {
...
void MCMC_state::update_G(const Dat *dat) {
	for (size_t i = 0; i < dat->n_ind; i++) {
    double G_i = 0.0;
...
void MCMC_state::sample_beta(size_t j, const Dat *dat, ldmat_data &ldmat_dat,unsigned sz){
    ...
    // A = res + X1 * beta1 + X2 * beta2
    gsl_vector *A = gsl_vector_alloc(dat->n_ind);
    ...
    gsl_blas_dgemv(CblasTrans, 1.0, X1, &beta1_here.vector, 1.0, A);  // A += X1^T * beta1
	//std::cout << "A after X1: " << gsl_vector_get(A, 0) << std::endl;
	
    gsl_blas_dgemv(CblasTrans, 1.0, X2, &beta2_here.vector, 1.0, A);  // A += X2^T * beta2
	...
	for (size_t i = 0; i < sub_size; i++) {
		...
		for (size_t k = 0; k < dat->n_ind; ++k) {
			dot_product1 += tau * gsl_vector_get(A, k) * dat->geno1[causal_list[i]][k];
			dot_product2 += tau * gsl_vector_get(A, k) * dat->geno2[causal_list[i]][k];
		}
    ...
    for (int i = 0; i < 3; i++) {
        for (int k = 0; k < 3; k++) {
            ...
            gsl_matrix_scale(&submat.matrix, cov_values[i][k]);
        }
	
    }
	
	
    // Step 4: Compute the inverse of the variance matrix
    
    gsl_permutation *p = gsl_permutation_alloc(mat_size);
    int signum;
	
    gsl_linalg_LU_decomp(var_mat, p, &signum);
	//std::cout << "var mat"<<gsl_matrix_get(var_mat,0,0)<< std::endl;
    
    gsl_linalg_LU_invert(var_mat, p, var_mat);
	//std::cout << "inverse"<<gsl_matrix_get(var_mat,0,0)<< std::endl;
	...
	//std::cout << "decomp result"<<gsl_matrix_get(result,0,0)<< std::endl;
    // \mu = L^{-1} A_vec
	...
	gsl_blas_dgemv(CblasTrans, -1.0, X1, &beta1_here.vector, 1.0, A);
	//std::cout << "A after X1 update: " << gsl_vector_get(A, 2) << std::endl;
	gsl_blas_dgemv(CblasTrans, -1.0, X2, &beta2_here.vector, 1.0, A);
	//std::cout << "A after X2 update: " << gsl_vector_get(A, 2) << std::endl;
	//max_a = 0.0;
	for (size_t m=0; m<dat->n_ind; m++) {
		residual[m] = gsl_vector_get(A, m);
...
void MCMC_state::sample_assignment(size_t j, const Dat *dat, const ldmat_data &ldmat_dat){

    size_t start_i = dat->boundary[j].first;
    size_t end_i = dat->boundary[j].second;
	int num_snp = end_i-start_i;
	double det_sigma = 1-rho*rho-rho_a*rho_a-rho_b*rho_b+2*rho*rho_a*rho_b;
	//std::cout <<n_cluster<<" "<< num_snp<< std::endl;
	...
	for (int k = 1; k < n_cluster; ++k) {
        ...
        for (int i = 0; i < num_snp; ++i) {
            ...
            gsl_matrix_set(A, 2, 2, dat->n_ind*eta*eta*Bii+(1-rho*rho)/deno_k);//ak3
            ...
            gsl_vector_set(B_i, 2, dat->n_ind * b3[start_i+i]);
            ...
            non_exp += std::log(p[1] / p[0]) + std::log(pi[k] + 1e-40);
...
void mcmc(Dat *dat, std::string out_path, int iter, int burn, double maf,unsigned sz) {
    ...
    for (size_t n_mcmc=0; n_mcmc<iter; n_mcmc++) {
		...
		for (size_t i=0; i<ref_size; i++) {
			...
			state.sample_assignment(i,dat,ldmat_dat);
